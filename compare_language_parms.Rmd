...
diag(comp_cors) = NA
#bad_lang = c("Mandarin (Taiwanese)","Latvian")
heatmap.2(comp_cors[-c(7,9),-c(7,9)], col=Colors)
...
long_list = ul[which(ul>1)] # 639
short_list = ul[which(ul>5)] # 386
...
comp_pars$sd = apply(comp_pars[,4:17], 1, sd, na.rm=T)
comp_pars$numNAs = apply(comp_pars[,4:17], 1, function(x) { sum(is.na(x)) })

med_d = median(comp_pars$sd, na.rm=T) # 2.18
sd_d = sd(comp_pars$sd, na.rm=T) # .97
...
diag(prod_cors) = NA
heatmap.2(prod_cors[-c(7,9),-c(7,9)], col=Colors)
```
...
prod_pars$sd = apply(prod_pars[,4:17], 1, sd, na.rm=T)
prod_pars$numNAs = apply(prod_pars[,4:17], 1, function(x) { sum(is.na(x)) })


med_d = median(prod_pars$sd, na.rm=T) # 2.14
sd_d = sd(prod_pars$sd, na.rm=T) # .68
