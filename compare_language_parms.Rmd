...
heatmap.2(comp_cors[-7,-7], col=Colors)
...
comp_pars$sd = apply(comp_pars[,4:18], 1, sd, na.rm=T)
comp_pars$numNAs = apply(comp_pars[,4:18], 1, function(x) { sum(is.na(x)) })
...
diag(prod_cors) = NA
heatmap.2(prod_cors[-7,-7], col=Colors)
```
...
prod_pars$sd = apply(prod_pars[,4:18], 1, sd, na.rm=T)
prod_pars$numNAs = apply(prod_pars[,4:18], 1, function(x) { sum(is.na(x)) })
