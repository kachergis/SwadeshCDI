...
#              "Latvian", "Norwegian", "French (Quebecois)", "Hebrew", # hebrew cause probs?
#              "Slovak", "Spanish (European)", "Russian", "Turkish", "English (British)",
#              "Portuguese (European)") # "Kigiriama" "British Sign Language"

languages = setdiff(names(coefs), c("Kigiriama"))
...
# 58 Croatian items, 79 French (French) items, 1 Hebrew, 2 Slovak, 103 Turkish..

# maybe just re-do all with item_id in coefs?

# ASSUMES SAME ORDER OF DEFINITIONS IN MIRT (should be true)
bind_lang_coefs <- function(languages, coefs) {
  xldf <- tibble()
  for(lang in languages) {
    load(paste("data/",lang,"_WG_data.Rdata", sep=''))
    #ldat <- items %>% left_join(coefs[[lang]]) # either by "definition" or "item_id"
    ldat <- items %>% distinct(item_id, definition, language, form, 
                               type, category, lexical_class, uni_lemma) %>% 
      bind_cols(coefs[[lang]] %>% select(a1, d))
    xldf <- bind_rows(xldf, ldat)
  }
  return(xldf)
}
...
for(lang in names(coefs)) {
  if("definition" %in% names(coefs[[lang]])) {
    if(grepl("item_", coefs[[lang]]$definition[1])) {
      ...
      print(paste(lang, "- renamed definition to item_id"))
    } 
  } else if("item_id" %in% names(coefs[[lang]])) {
    if(! grepl("item_", coefs[[lang]]$item_id[1])) {
      coefs[[lang]] <- coefs[[lang]] %>% rename(definition = item_id)
      print(paste(lang, "- renamed item_id to definition"))
    }
  }
}
...
#  "Kigiriama - renamed definition to item_id"
# "Spanish (Mexican) - renamed item_id to definition" (why was this one messed up?)
# "English (British) - renamed definition to item_id"
# "Hebrew - renamed definition to item_id"
# "Portuguese (European) - renamed definition to item_id"
...
# Column `Spanish (Mexican)` doesn't exist. ???
require(gplots)
...
# "Korean - renamed definition to item_id"
# "Spanish (Mexican) - renamed definition to item_id"
# "French (French) - renamed definition to item_id"
# "Latvian - renamed definition to item_id"
# "Hebrew - renamed definition to item_id"
# "Slovak - renamed definition to item_id"
# "Spanish (European) - renamed definition to item_id"
# "Kigiriama - renamed definition to item_id"
# "Kiswahili - renamed definition to item_id"
# "Mandarin (Beijing) - renamed definition to item_id"
# "Portuguese (European) - renamed definition to item_id"
# "English (British) - renamed definition to item_id"
xldf <- join_lang_coefs(languages, coefs) 
