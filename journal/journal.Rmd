...
languages = subset(all_tab, N>=300)$Language
low_data_langs <- it_tab %>% filter(`N`<300)
...
We pulled data for 42 languages from Wordbank [@frank2017].
...
xldf_lowd <- xldf_clean %>% filter(is.element(language, low_data_langs$Language)) # 5624
xldf <- xldf_clean %>% filter(!is.element(language, low_data_langs$Language)) # 20015
