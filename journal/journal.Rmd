...
```{r report-wg-ws-demo-data-old, includ=F, echo=F}
# report mean age and N per language (by form type?)
...
Summarized in Table 1, the Wordbank[^2] datasets consisted of production data for `r sum(model_stats$participants)` children aged 8--30 on `r sum(model_stats$items)` items across `r nrow(model_stats)` forms.
...
full_fscores <- readRDS(here("data/full_fscores_allforms.rds"))
