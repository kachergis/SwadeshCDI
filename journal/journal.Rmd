...
model_stats <- readRDS(here("data/model_stats.rds"))
...
load(here("data/Swadesh_grid_search_final.Rdata")) # ToDo: update / remove!
...
not_en_good <- subset(en_good_prod, is.na(uid)) # 28 
...
#all_fscores <- readRDS(here("data/full_fscores_allforms.rds"))
...
```{r, out.width="\\linewidth", eval=F, fig.align="center", fig.cap=c("The 229 Swadesh CDI concepts by semantic category, and 10-item extension (in red). Italics denote the 28 items not included on the American English CDI:WS."), echo=FALSE}
...
good_prod2 <- good_prod %>% left_join(xldf %>% distinct(uni_lemma, category)) # FixMe: , lexical_category
...
On average, the `r length(train_langs)` CDI forms included $N=176$ of the Swadesh items.
...
For the `r length(gen_langs)` low-data languages, a comparison of simulated S-CDI scores to full CDI:WS revealed that the S-CDI's raw scores were again strongly related (mean $r=0.990$), with an average of $N=153$ S-CDI items appearing per list.
...
xx_it <- run_comparisons(xldf, train_langs, good_prod$uni_lemma, 
                         rand_method="items", metrics=c("sumscore_cor","test_info"))

# with 10-item extension
xx_it_ext <- run_comparisons(xldf, train_langs, c(swad10, good_prod$uni_lemma), 
                         rand_method="items", metrics=c("sumscore_cor","test_info"))
...
# just the Swadesh
xx <- run_swadesh_comparisons(xldf, train_langs, good_prod$uni_lemma)
...
gen_xx_it <- run_comparisons(xldf_clean, gen_langs, good_prod$uni_lemma, 
                              rand_method="items", metrics=c("sumscore_cor"))

gen_xx_it_ext <- run_comparisons(xldf_clean, gen_langs, c(swad10, good_prod$uni_lemma), 
                              rand_method="items", metrics=c("sumscore_cor"))
...
gen_xx <- run_swadesh_comparisons(xldf_lowd, gen_langs, good_prod$uni_lemma)
...
xx10 <- run_swadesh_comparisons(xldf, train_langs, c(swad10, good_prod$uni_lemma))
mean(xx10$`Swadesh r`)
# .996 # 190 available vs 176 

gen_xx10 <- run_swadesh_comparisons(xldf_lowd, gen_langs, c(swad10, good_prod$uni_lemma))
