...
The CDI Words & Gestures form (CDI:WG) targets children 8-18 months of age, typically consisting of a subset of approximately 500 of the easier items from the CDI:WS of the same language, and asks caregivers to report comprehension as well as production. 
...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts in Wordbank, 2) to fit IRT models to 27 CDI:WS datasets, 3) to evaluate candidate lists of Swadesh CDI items from a cross-linguistic comparison of concept difficulty and inclusion, 4) to identify and characterize the most informative Swadesh CDI list of XX [FixMe] concepts, and 5) to test its generalization to a set of 11 additional low-data languages. 
...
ToDo: explain stitching of WG and WS

We report IRT analyses for twenty-seven languages from Wordbank [@frank2017], comprising production data from CDI:WS vocabulary checklists that have at least 200 administrations.[^2]
...
ToDo: update to report total CD:WS and CDI:WG Ns?

The Wordbank datasets consisted of the CDI:WS production data for `r sum(all_tab$N)` children aged 16--30 months on `r sum(all_tab$items)` items across `r nrow(all_tab)` forms.
