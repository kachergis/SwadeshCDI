...
# how many of the initial items remain?
...
# 77 items on 2/3 lists

all3 <- intersect(l1_l2, l2_l3) # 14 on all 3

swad_unis <- unis[on2lists,]$uni_lemma

unis <- unis %>% mutate(Swadesh = ifelse(is.element(uni_lemma, swad_unis), T, F))

unis %>% group_by(Swadesh) %>%
  summarise(d_m = mean(d_m), n = mean(n), a1_m = mean(a1_m))
