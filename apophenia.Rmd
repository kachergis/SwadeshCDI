...
category_proportions # 26 proportions <- will round up smaller categories and round down largers ones..
n_per_category <- category_proportions * n_items
...
make_swadesh_list <- function(unis, n_items=100, iters=1000, 
  ...
  if(is.null(prev_run)) {
    ...
    cur_swad_inds = c()
    for(ci in n_per_category) {
      this_cat_uni <- subset(unis, category==n_per_category[ci,]$category)
      new_unis = sample(this_cat_uni$uni_lemma, 
                             n_per_category[ci,]$n, replace = F)
  ...
  for (i in 1:iters) {
    ...
    drop_unilemma = unis[cur_swad_inds[drop_i],]$unilemma
    # get item from same category
    new_unilemma = sample(subset(unis, category==drop_cat & unilemma!=drop_unilemma), 1)
    new_i = which(unis$unilemma==new_unilemma)
