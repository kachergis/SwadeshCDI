...
```{r, include=F, echo=F}
length(intersect(r1$swad_inds[1,], r1$swad_inds[lasti,])) # 17 remain
length(intersect(r2$swad_inds[1,], r2$swad_inds[lasti,])) # 15
length(intersect(r3$swad_inds[1,], r3$swad_inds[lasti,])) # 15
length(intersect(r4$swad_inds[1,], r4$swad_inds[lasti,])) # 24
```
...
`r on4` uni-lemmas are on all 4 chains.
`r on3` uni-lemmas are on 3 chains, `r on2` are on 2 chains.
`r on1` are only on 1 chain.

## Items on all 4 chains
...
unis[as.numeric(names(xc_freq[which(xc_freq==4)])),]$uni_lemma
```


## Items on 3 of 4 chains


```{r, echo=F}
unis[as.numeric(names(xc_freq[which(xc_freq==3)])),]$uni_lemma
```

## Properties of items on most/all chains

```{r, echo=F, warning=F}
sinds = as.numeric(names(xc_freq[which(xc_freq>2)]))
