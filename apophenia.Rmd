...
make_swadesh_list <- function(unis, n_items=100, iters=1000, cur_list=c()) {
  ...
  for (i in 1:iters) {
    ...
    # swap in 1 new item
    swad_inds2 = c(swad_inds[1:(n_items-1)], sample(setdiff(1:nrow(unis), swad_inds), 1))
    ...
    if(l1r < l2r && runif(1)<=.95) swad_inds = swad_inds2
    cors[i] = max(l1r, l2r)
  }
...
# do more iterations:
#result <- make_swadesh_list(unis, n_items=100, iters=10000, cur_list=result$swad_list)
...
length(intersect(result$swad_list, result2$swad_list))
length(intersect(result2$swad_list, result3$swad_list))
length(intersect(result$swad_list, result3$swad_list))
```
