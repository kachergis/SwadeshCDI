---
title: "Swadesh goes to Monte Carlo"
author: "George & Alvin"
date: "`r Sys.Date()`"
...
combine_res <- function(res_list) {
  ...
  list(all_cor = all_cor,
       all_val = all_val)
}

combine_plot <- function(combined_res) {
  all_cor <- combined_res$all_cor
  all_val <- combined_res$all_val
...
combined1 <- combine_res(list(result1, result1_2, result1_3))
combined2 <- combine_res(list(result2, result2_2, result2_3))
combined3 <- combine_res(list(result3, result3_2, result3_3))
combined4 <- combine_res(list(result4, result4_2, result4_3))

combine_plot(combined1)
combine_plot(combined2)
combine_plot(combined3)
combine_plot(combined4)
```
...
How peaky is the posterior?

```{r}
r1 <- readRDS("data/MC/result1.rds")
r2 <- readRDS("data/MC/result2.rds")
r3 <- readRDS("data/MC/result3.rds")
r4 <- readRDS("data/MC/result4.rds")

# every 100 iterations, current Swadesh list is saved (but same one for Hungarian and Finnish)

all_inds1 <- r1$all_val %>% filter(val_lang=="Finnish", i>5000) %>% select(swad_inds) %>% unlist()
all_inds2 <- r2$all_val %>% filter(val_lang=="Finnish", i>5000) %>% select(swad_inds) %>% unlist()
all_inds3 <- r3$all_val %>% filter(val_lang=="Finnish", i>5000) %>% select(swad_inds) %>% unlist()
all_inds4 <- r4$all_val %>% filter(val_lang=="Finnish", i>5000) %>% select(swad_inds) %>% unlist()

all_inds <- sort(table(c(all_inds1, all_inds2, all_inds3, all_inds4)), decreasing=T)
#all_inds <- c(all_inds1, all_inds2, all_inds3, all_inds4)
#hist(all_inds)
item_posterior_prob = all_inds / sum(all_inds)

head(all_inds, 10)
```

`r length(all_inds)` uni-lemmas appear in the four chains, with a median frequency of `r median(all_inds`.
This means we haven't even fully sampled the space of items (let alone item combinations..); we probably need a higher mutation acceptance rate to more fully explore it.
