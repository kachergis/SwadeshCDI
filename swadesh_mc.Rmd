...
# ideas: maximize correlation OR minimize sd(sorted difficulty differences),
# minimize variance of xling difficulty (as long as we get >>2 items)
# (OR maximize number of forms -- but then we get easy items)
objective <- function(xx_g) {
  spacing <- sd(sort(diff(xx_g$difficulty)))
  var(unis)
  # var(diff) and spacing 
}
# does convergence happen for each of these alone? (each of these will bias it somehow -- e.g., # of forms = too easy;)
# 
...
make_swadesh_list <- function(unis, languages, n_items = 100, iters = 1000, 
  ...
  for (i in 1:iters) {
    ...
    l1r = objective(xx_g) # mean(xx_g$`Swadesh r`) 
    l2r = objective(xx_g2) # mean(xx_g2$`Swadesh r`)
...
```{r}
r1 <- make_swadesh_list(unis, train_langs, n_items = 100, iters = 3000)
r2 <- make_swadesh_list(unis, train_langs, n_items = 100, iters = 3000)

intersect(r1$swad_inds[])
```
