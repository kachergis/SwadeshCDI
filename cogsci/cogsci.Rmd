...
abstract: >
    ...
    existing CDIs. Leveraging 26 datasets from existing CDIs, we propose a list of 108 concepts that have
...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts ("universal lemmas", i.e. uni-lemmas) in Wordbank, 2) to fit IRT models to 28 CDI:WS datasets, 3) to identify 108 candidate "Swadesh CDI" items from a cross-linguistic comparison of concept difficulty and inclusion, and 4) to test how well this Swadesh CDI generalizes to a set of 8 additional low-data languages. 
...
<!-- Figure 1 shows the average difficulty of items in each language, but as mentioned above these differences should not be interpreted, as they are at least in part related to differences in the samples. 
Indeed, the average age of the sample was significantly related to the average difficulty of words on a CDI:WS form ($r$=.57; $t(24)=3.44$, $p=.002$).
However, even with balanced samples in terms of age, sex, and maternal education, there may well be differences in overall difficulty of words on the CDI:WS. 
Fortunately, all that is required for our current goals is to be able to rank the words in terms of their cross-linguistic variability in difficulty, which should not be systematically influenced by random effects of sample or language. -->
...
#uni_ag %>% filter(n>34) 16 uni-lemmas appear more than on some forms: pronouns,
# note: 3SG   (pronoun) has 80 occurrences (on 34 forms)  mean d_m=1.92  (d_sd=1.34)

# The more languages a word is in, the easier it tends to be:
cor.test(uni_ag$d_m, uni_ag$n)
# r=-.27, t(2440) = -13.91, p<.001
...
# r=.16, t(1450) = 6.313,  p<.001
# The more forms a uni-lemma appears on, 1) the easier it tends to be (r=-.27), and 2) the higher the variability in its difficulty (r=.156.

# 528 singletons
uni1 <- uni_ag %>% filter(n==1) 
mean(uni1$d_m) # -1.44

uni_ag <- uni_ag %>% filter(n>1)
...
# regression to control for per-language mean shift in difficulty?
```
...
med_d = median(prod_pars$sd, na.rm=T) # 1.20
# SD of the SD
sd_d = sd(prod_pars$sd, na.rm=T) # .42
...
```{r missing-by-sd, include=F, fig.env = "figure", fig.pos = "h", fig.height=3.5, fig.align = "center", fig.cap = "Median cross-linguistic variability in uni-lemma difficulty of CDI words by exclusion threshold (number of forms from which uni-lemma is missing)."}
good_prod <- prod_pars %>% 
  filter(numNAs < 16) %>%
...
for(k in 2:25) {
  ...
  # correlation bw Swadesh and full CDI
  # could also do for low_data_langs...
  uni_k_eval <- run_swadesh_comparisons(xldf, languages, 
                                        swad_lists[[k]]$uni_lemma, form='WS', rand_comparisons=0)
  swad_mean_diff = mean(swad_lists[[k]]$d_m)
  swad_diff_sd = sd(swad_lists[[k]]$d_m)
  swad_n = nrow(swad_lists[[k]])
  swad_cor = mean(uni_k_eval$`Swadesh r`)
  swad_n_present = mean(uni_k_eval$N) # mean number of swadesh items on each form
  
  # test information
  total_tinfo <- get_test_information(xldf, languages, swad_lists[[k]]$uni_lemma, form="WS")
  swad_tinfo_m <- mean(subset(total_tinfo, sublist=="Swadesh")$total_tinfo)
  swad_tinfo_sd <- sd(subset(total_tinfo, sublist=="Swadesh")$total_tinfo)
  rand_tinfo_m <- mean(subset(total_tinfo, sublist=="random")$total_tinfo)
  rand_tinfo_sd <- sd(subset(total_tinfo, sublist=="random")$total_tinfo)
  
  swad_agg = bind_rows(swad_agg, 
                       tibble(k, N, mean_diff, mean_sd, sd_sd, # all uni-lemmas on >=k forms
                              swad_n, swad_mean_diff, swad_diff_sd, swad_cor, swad_n_present,
                              swad_tinfo_m, swad_tinfo_sd, rand_tinfo_m, rand_tinfo_sd))
}
...
library(wordcloud2)
png("Swadesh-wordcloud.png", width=7, height=7)
wordcloud2(data=data.frame(word = names(swad_freq), 
                           freq = as.vector(swad_freq)), size=1.6, color='random-dark')
dev.off()
# some discrepancies between this and the good_prod approach -- just because we're not slicing the highest discrimination item??
...
# mean difficulty of Swadesh words: -0.10; mean easiness of other words: 0.53
...
# t = 15.627, df = 3122.9, p-value < 2.2e-16
...
# just the Swadesh 108
xx <- run_swadesh_comparisons(xldf, languages, good_prod$uni_lemma)
#summary(xx$`Swadesh r`)
# Min.    1st Qu.  Median   Mean 3rd Qu.    Max. 
# 0.9788  0.9877  0.9900  0.989  0.992  0.996
#summary(xx$`Rand r`)
#  Min.  1st Qu.  Median Mean  3rd Qu.    Max. 
# 0.991  0.993  0.994   0.994  0.995   0.996 

# Swadesh 108 + 10 harder words 
xx_ext <- run_swadesh_comparisons(xldf, languages, c(good_prod$uni_lemma, low_var$uni_lemma))
# summary(xx_ext$`Swadesh r`)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.9788  0.9890  0.9914  0.9908  0.9930  0.9962 
#summary(xx_ext$`Rand r`)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.9906  0.9932  0.9941  0.9940  0.9955  0.9965 
...
gen_xx_ext <- run_swadesh_comparisons(xldf_lowd, gen_test_langs, c(good_prod$uni_lemma, low_var$uni_lemma))
#summary(gen_xx_ext$`Swadesh r`) 
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.9669  0.9811  0.9849  0.9837  0.9899  0.9931 

# summary(gen_xx_ext$`Rand r`)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.9784  0.9889  0.9901  0.9891  0.9910  0.9940 

save(xx, gen_xx, xx_ext, gen_xx_ext,  
...
```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("The 108 proposed Swadesh CDI words by semantic category."), echo=FALSE}
...
We identified 108 concepts that appeared on at least 10 of the CDIs, and which have more consistent cross-linguistic difficulty than the majority of the concepts appearing on multiple CDIs.
...
Despite the myriad challenges that remain in creating new measures of early language development, we believe that the proposed Swadesh CDI list will give researchers a solid foundation to start from.
