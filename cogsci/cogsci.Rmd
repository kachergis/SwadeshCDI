...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts ("universal lemmas", i.e. uni-lemmas) in Wordbank, 2) to fit IRT models to 28 CDI:WS datasets, 3) to identify 229 candidate "Swadesh CDI" items from a cross-linguistic comparison of concept difficulty and inclusion, and 4) to test how well this Swadesh CDI generalizes to a set of eight additional low-data languages. 
...
Of those items, 95% had uni-lemmas defined; a median of 693 per CDI:WS form (range: 553 (Czech) to 804 (Cantonese)). 
...
However, these modest but significant correlations are also important to keep in mind as we choose our Swadesh CDI candidates, as selecting too many easy items could result older children being at ceiling.
...
plot_k_n <- ggplot(data = swad_agg |> select(k, N, swad_n) |> distinct()) +
  ...
  geom_line(aes(x = k, y = swad_n), color="blue", linetype="dashed") + 
  ...
  theme_classic() +
  scale_x_continuous(breaks = c(5,10,15,20,25)) + ylab("Number of uni-lemmas")
...
ggarrange(plot_k_n, plot_k_testinfo, nrow=2)
```
