...
```{r grid-search-plots, fig.cap=c("Number of uni-lemmas appearing on at least $k$ lists (top). Total test information for Swadesh lists, and random tests of the same length (bottom)."), fig.height=5.0, fig.width=3.4}
...
Finally, 28 S-CDI uni-lemmas were not present on the American English CDI:WS (see italicized items in Fig. 3). <!--, suggesting that this method may help us move beyond an English-centric approach to studying early language. -->
...
```{r difficulty-by-category, fig.env = "figure", fig.pos = "h", fig.height=3.7, fig.align = "center", fig.cap = "Mean cross-linguistic difficulty of CDI words by semantic category, showing that selection of Swadesh concepts broadly maintained representative difficulty. Bars represent bootstrapped 95\\% confidence intervals."}
...
However, S-CDI items were significantly easier than the other uni-lemmas (mean S-CDI $d=$ `r round(tmp[2,"d"], 2)`, other items' mean $d=$ `r round(tmp[1,"d"], 2)`, `r report_ttest(swad_diff_ttest)`).
