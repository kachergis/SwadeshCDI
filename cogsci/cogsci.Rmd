...
abstract: >
    ...
    existing CDIs. Leveraging 26 datasets from existing CDIs, we propose a list of 106 concepts that have
...
While other standard IRT models exist (e.g., the 3-parameter logistic model adds a 'guessing' parameter for each test item), recent studies have found the 2PL model most appropriate for Wordbank data [@Kachergis2022].
...
```{r fig-mean-item-diff, fig.cap=c("Mean item difficulty for each CDI:WS form. Bars represent bootstrapped 95\\% confidence intervals. Color shows mean age (months) of the sample, which is correlated with mean word difficulty (r=.57)."), fig.height=4.3, fig.width=3.4}
...
[^3]: OSF repository: [https://osf.io/XXX/](https://osf.io/XXX/).
