...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts in Wordbank, 2) to fit IRT models to 28 CDI:WS datasets, 3) to identify a list of 229 "Swadesh CDI" items from a cross-linguistic comparison of concept difficulty and inclusion, and 4) to test how well this Swadesh CDI generalizes to a set of eight additional low-data languages. 
...
For $v_{min}$, we simply look at the uni-lemmas with half a standard deviation less variation in cross-linguistic difficulty than average, among the set of uni-lemmas that are on at least $k$ forms.
...
Thus, the higher the threshold $k$, the more the Swadesh candidates will be biased to be easier, and the less _universal_ they may be.
This could result in a Swadesh list that is overall easier than the CDI:WS (and thus may show ceiling effects for older children), and that is less universal in difficulty than, say, a random selection of uni-lemmas.
...
The `r nrow(good_prod)` Swadesh-CDI items represents 21 of the 22 semantic categories present on the original American English CDI:WS form, with action words, animals, household, food/drink, body parts, and descriptive words being most prevalent, and connecting words, quantifiers, helping verbs, and articles being rare, and question words being unrepresented.
54% of the Swadesh CDI concepts are nouns, 22% are predicates, 17% are function words, and 7% are other.
...
Finally, 28 S-CDI uni-lemmas are not present on the American English CDI:WS (e.g., 'fruit', 'ambulance', 'enter'), suggesting that this method may help us move beyond an English-centric approach to studying early language.
...
```{r difficulty-by-category, fig.env = "figure", fig.pos = "h", fig.height=4.0, fig.align = "center", fig.cap = "Mean cross-linguistic difficulty of CDI words by semantic category. Bars represent bootstrapped 95\\% confidence intervals."}
...
These correlations were compared to a random baseline in which uni-lemmas were selected from the set of unique uni-lemmas, weighted by the frequency of that uni-lemma appearing on the existing CDI:WS forms. 
The corelations of scores on these random vs. the full CDI:WS were quite similar to the Swadesh list (TODO SHOW THESE), but the random tests also yielded lower total test information.
...
## Generalization Test of the Swadesh CDI 

For the eight low-data languages, a similar comparison revealed that the Swadesh CDI's raw scores were again strongly related to the full CDI:WS scores (mean $r=0.990$; $min=0.983$, $max=0.997$).
...
We identified 229 concepts that appeared on at least 9 of the CDIs, and which have more consistent cross-linguistic difficulty than the majority of the concepts appearing on multiple CDIs.
