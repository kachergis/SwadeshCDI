...
Across the 26 IRT models for different CDI:WS forms, diffictulty and discrimination parameters for a total of 23020 items were fitted.
Of those items, 95% have uni-lemmas defined; a median of 693 per CDI:WS form (range: 553 (Czech) to 804 (Cantonese)). 
There are a total of `r length(unique(xldf$uni_lemma))` uni-lemmas defined across the 26 CDI:WS forms, but `r nrow(uni1)` of these are singletons, appearing on only one of the 26 forms.
...
However, these modest but significant correlations are also important to keep in mind as we choose our Swadesh CDI candidates: selecting too many easy items could result older children being at ceiling.
...
The goal is to choose uni-lemmas that have low variability in their cross-linguistic difficulty -- that is, uni-lemmas that are similarly difficult-to-learn across languages, as this is our operationalization of _universal_.
There are two threshold that must be defined to select Swadesh CDI (S-CDI) candidates: 1) $k$, the number of current CDI:WS forms a uni-lemma must appear on, and 2) $v_{min}$, the threshold for variability in cross-linguistic difficulty. 
For $v_{min}$, we simply look at the uni-lemmas with half a standard deviation less cross-linguistic difficulty than average, among the set of uni-lemmas that are on at least $k$ forms.
...
For each $k$, we found the set of uni-lemmas that meet 
We evaluate total test information, 
...
Figure 1 shows the average cross-linguistic difficulty of CDI items by semantic category, for both Swadesh and non-Swadesh uni-lemmas.
...
```{r grid-search-plots}
plot_k_n <- ggplot(data = swad_agg |> select(k, N) |> distinct()) +
...
Of these, a total of `r nrow(uni_ag_gt1)` uni-lemmas are included on more than one form, and only `r subset(swad_agg,k==9)$N[1]` uni-lemmas are included on 9 or more of the CDIs.
...
The mean SD is `r round(swad_info$mean_sd[1], 2)` (SD=`r round(swad_info$sd_sd[1], 2)`), so we consider the items with SD less than half the mean SD (i.e., SD < `r round(swad_info$sd_thresh, 2)`).
These `r nrow(good_prod)` candidate "Swadesh CDI" items are shown in Figure 3, grouped by semantic category.
The Swadesh CDI candidates represent ?? of the 22 semantic categories on the original American English CDI:WS form: the five categories not represented are toys, pronouns, question words, quantifiers, and helping verbs.
...
The Swadesh candidates were also present on more forms than typical in the selection set: on average, each item appeared on `r round(mean(good_prod$n), 0)` forms.
...
# Swadesh words have higher discrimination when k=16, but same disc when k=9

#t.test(subset(xldf, SwadeshCDI==1)$d, subset(xldf, SwadeshCDI==0)$d)
# t = -13.089, df = 8749, p-value < 2.2e-16
...
# [1] "airplane" "all gone" "bed"      "beside"   "broom"    "candy"    "cat"      "chin"     "cold"     "dog"      "duck"    
...
(Actually lower than expected by chance? 27%, and we have selected 34% of the length of the EN CDI:WS -- oh, actually, how many of the Swadesh concepts are on the EN WS? interesting discussion point, re: English-centric bias)

```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("NOT UPDATED The most frequently-selected 100 Swadesh CDI concepts by semantic category."), echo=FALSE}
