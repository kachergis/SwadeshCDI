...
=======
    ...
    low variability in their cross-linguistic learning difficulty. This pool of 'universal' concepts--
    analogous to the "Swadesh" lists used in glottochronology--can be used as a starting point for future 
>>>>>>> 7328cebec0f20f24ec18020ac7fe9fb1f3a5b7e5
...
# are singletons harder than unis appearing on multiple forms?
t.test(uni1$d_m, uni_ag_gt1$d_m)
# t(899) = 7.66, p<.001


# no cor bw difficulty and variability -- good
cor.test(uni_ag_gt1$d_m, uni_ag_gt1$d_sd)
...
However, we must define two thresholds for selecting Swadesh CDI (S-CDI) candidates: 1) the number of current CDI:WS forms a uni-lemma must appear on, and 2) the threshold for variability in cross-linguistic difficulty.

Before we select these thresholds, we characterize the overall set of uni-lemmas.
There are `r length(unique(xldf$uni_lemma))` unique uni-lemmas across the 26 CDI:WS forms, with an average difficulty of $d=$ `r mean(uni_ag$d_m)`.
Of these, `r nrow(uni1)` uni-lemmas are singletons, appearing on only one of the 26 forms.
These singletons are significantly more difficult than the `r nrow(uni_ag_gt1)`` uni-lemmas appearing more than once ($M_1=1.45$; $M_{>1}=0.85$ $t(899) = 7.66$, p<.001).
Indeed, there is a significant relation between the number of forms a uni-lemma appears on and its difficulty: the more forms a uni-lemma is on, the _easier_ it tends to be ($r=-.37$, $t(1837) = 17.15$, $p<.001$).
Moreover, there is a weak but significant relation between the number of forms a uni-lemma appears on and its cross-linguistic variability ($r=.17$, $t(1309)=6.35$, $p<.001$)
Thus, in selecting the Swadesh candidates, we will need to consider that by thresholding the number of forms a uni-lemma must appear on, we are biasing our Swadesh candidates to be easier, and to have greater cross-linguistic variability in difficulty.
...
```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("The 229 proposed Swadesh CDI words by semantic category."), echo=FALSE}
