...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts ("universal lemmas", i.e. uni-lemmas) in Wordbank, 2) to fit IRT models to 28 CDI:WS datasets, 3) to identify 106 candidate "Swadesh CDI" items from a cross-linguistic comparison of concept difficulty and inclusion, and 4) to test how well this Swadesh CDI generalizes to a set of 8 additional low-data languages. 
...
# r=-.40, t = -15.705, df = 1308, p-value < 2.2e-16
...
# The more forms a uni-lemma appears on, 1) the easier it tends to be (r=-.40), and 2) the higher the variability in its difficulty (r=.15).
...
# SD of the SD
sd_d = sd(prod_pars$sd, na.rm=T) # .41
...
54% of the Swadesh CDI words are nouns, 20% are predicates, 5% are function words, and 21% are other.
...
```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("The 106 proposed Swadesh CDI words by semantic category."), echo=FALSE}
...
We identified 106 concepts that appeared on at least 10 of the CDIs, and which have more consistent cross-linguistic difficulty than the majority of the concepts appearing on multiple CDIs.
