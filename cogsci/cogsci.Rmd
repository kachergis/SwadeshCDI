...
These mappings were recently updated to improve their quality and systematicity, and to increase coverage across items and languages. <!-- @Mike can we use that new crypto-ID to cite new WB2? -->
...
A total of `r length(unique(xldf$uni_lemma))` uni-lemmas were defined across the 26 CDI:WS forms, but `r nrow(uni1)` of these were singletons, appearing on only one of the 26 forms.
These singletons were significantly more difficult than the `r nrow(uni_ag_gt1)` uni-lemmas appearing more than once ($M_1=$ `r round(mean(uni1$d_m),2)`; $M_{>1}=$ `r round(mean(uni_ag_gt1$d_m),2)`; `r report_ttest(t.test(uni1$d_m, uni_ag_gt1$d_m))`).
...
It is perhaps intuitive that more "universal" concepts---which were thus selected to be on more CDI forms---also tended to be earlier learned, and this echoes earlier work characterizing the consistency of children's first words across several languages [@tardif2008baby].
...
The goal was to choose uni-lemmas that have low variability in their cross-linguistic difficulty---that is, uni-lemmas that are similarly difficult to learn across languages.
...
This criterion will preserve the XX% least variable of the uni-lemmas. <!-- explain what % of the distribution this keeps:  -->
...
These S-CDI items were strongly predictive of full CDI:WS scores, with an mean $r=0.995$.
The mean difficulty of the S-CDI items was also quite close to the difficulty of the other uni-lemmas in the set ($M(d_{Swad})=0.31$; $M(d_{other})=0.36$), suggesting that the S-CDI items were fairly representative.
...
Figure 2 shows the average cross-linguistic difficulty of CDI items by semantic category, grouped by Swadesh and non-Swadesh uni-lemmas. 
...
Comparing the IRT parameters of the S-CDI uni-lemmas to the rest of the items showed that the discrimination parameter (i.e., slope) of the Swadesh items did not significantly differ from the others, suggesting that the Swadesh items could measure ability as well as non-Swadesh uni-lemmas.
...
On average, for the 26 large CDI:WS datasets, the S-CDI's sumscores were strongly related to the full CDI:WS scores (mean $r=0.990$; $min=0.976$, $max=0.996$; full table on [OSF](https://osf.io/8swhb/?view_only=6f6ab9818f2a4bb288e05ca9e12f540c)).
...
This baseline had correlations to full CDI:WS scores of (TODO FILL IN VALUES).
...
As found above, the S-CDI yielded a total test information of (TODO FILL IN VALUES), whereas the English uni-lemmas baseline yielded a total test information of (TODO FILL IN VALUES), and the random uni-lemmas baseline yielded a total test information of (TODO FILL IN VALUES).
...
In particular, researchers should seek to add contextually appropriate items from the categories that were less well-represented on the Swadesh list (e.g., quantifiers, question words). 
These categories also tended to be more difficult, so adding items from them may help to raise the difficulty ceiling of the form.
