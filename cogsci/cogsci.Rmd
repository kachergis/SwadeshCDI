...
The vocabulary checklist from the American English CDI Words & Gestures (CDI:WG) form, targeting younger children ages 8 to 18 months, is comprised of $\sim400$ words from a similar set of categories. 
Items on these original forms were chosen to reflect a range of difficulty levels (i.e., easy, moderate, and more difficult), as well as capture the linguistic and societal contexts of (most) children living in the US.
...
<!-- rewrite/break up long sentence -->
...
The goal of the current study is to use IRT modeling in conjunction with data from Wordbank to examine whether there might be a core set of concepts that are frequently included on CDIs, and---importantly---whether enough of them are of roughly equal difficulty across many languages. 
...
For example, the original Swadesh list contains 100 words, comprised of categories including common pronouns (_I_, _you_, _we_), animals (_man_, _fish_, _bird_, _dog_), objects (_tree_, _leaf”, _sun_, _mountain_), and verbs (_die_, _see_, _sleep_, _kill_,). 
...
In particular, our contributions are 1) to revise and extend a set of translation-equivalent concepts in Wordbank, 2) to fit IRT models to 28 CDI:WS datasets, 3) evaluate 25 candidate lists of "Swadesh CDI" items from a cross-linguistic comparison of concept difficulty and inclusion, and 4) identified and characterized the most informative Swadesh CDI list of 229 concepts and tested its generalization to a set of eight additional low-data languages. 
...
tab1 <- xtable::xtable(it_tab, digits=c(0), 
                       caption = "Number of CDI:WS items and subjects (N) per dataset. The final eight datasets were used for a generalization test.")
...
Datasets for these languages will be used to test how well the selected pool can be expected to generalize to new languages.

### Uni-lemmas

In order to do so, we need a method to map between words that correspond to broadly similar concepts across languages. 
...
These mappings were recently updated for Wordbank 2.0 to improve their quality and systematicity, and to increase coverage across items and languages. <!-- @Mike can we use that new crypto-ID to cite new WB2? -->
...
Note that the distributions of demographic variables (age, sex, maternal education, etc.) of these datasets are not matched, so comparing overall language ability estimates across languages would be ill-advised. 
...
Of those items, 95% had uni-lemmas defined; a median of 693 per CDI:WS form (range: 553 in Czech to 804 in Cantonese). 
A total of `r length(unique(xldf$uni_lemma))` uni-lemmas are defined across the 26 CDI:WS forms, but `r nrow(uni1)` of these are singletons, appearing on only one of the 26 forms.
These singletons are significantly more difficult than the `r nrow(uni_ag_gt1)` uni-lemmas appearing more than once ($M_1=$ `r round(mean(uni1$d_m),2)`; $M_{>1}=$ `r round(mean(uni_ag_gt1$d_m),2)`; `r report_ttest(t.test(uni1$d_m, uni_ag_gt1$d_m))`).
Indeed, there is a significant relation between the number of times a uni-lemma appears and its difficulty: the more times a uni-lemma appears, the _easier_ it tends to be (`r report_cor(cor.test(uni_ag$d_m, uni_ag$n))`).
Moreover, there is a weak but significant relation between the number of forms a uni-lemma appears on and its cross-linguistic variability (`r report_cor(cor.test(uni_ag$d_sd, uni_ag$n))`).
It is perhaps intuitive that more 'universal' concepts---which are thus selected to be on more CDI forms---also tend to be earlier learned, and this echoes earlier work characterizing the consistency of children's first words across several languages [@tardif2008baby].
However, these modest but significant correlations are also important to keep in mind as we choose our Swadesh CDI candidates, as selecting too many easy items could result in older children being at ceiling.
...
The goal was to choose uni-lemmas that have low variability in their cross-linguistic difficulty---that is, uni-lemmas that are similarly difficult to learn across languages, as this is our operationalization of _universal_.
There are two thresholds that must be defined to select Swadesh CDI (S-CDI) candidates: 1) $k$, the number of current CDI:WS forms a uni-lemma must appear on, and 2) $v_{min}$, the threshold for variability in cross-linguistic difficulty. 
For $v_{min}$, we simply consider the uni-lemmas with half a standard deviation less variation in cross-linguistic difficulty than average, among the set of uni-lemmas that are on at least $k$ forms. This criterion will preserve the XX% least variable of the uni-lemmas. <!-- explain what % of the distribution this keeps:  -->

It less clear how to choose $k$, the minimum number of forms a uni-lemma must appear on to be considered eligible.
As reported above, the more forms a uni-lemma appears on, the easier that concept tends to be, and the greater its cross-linguistic variability in difficulty ($SD(d)$) tends to be.
Thus, the higher the threshold $k$, the more bias there will be toward the Swadesh candidates being easier than typical, as well as less _universal_.
Having a Swadesh CDI list that is much easier than the CDI:WS is undesirable, as it may show ceiling effects for older children, while selecting words that are less _universal_ may result in a test that generalizes less well to languages in comparison to, say, a random selection of uni-lemmas.
Of course, the higher $k$, the fewer uni-lemmas there are available to choose from: 1303 uni-lemmas appear on 2 or more CDIs, but only 30 appear on all 26 CDIs.
Because choosing $k$ is difficult _a priori_, we opted to do a grid search across all possible values. 
For each $k$, we found the set of N uni-lemmas appearing $k$ times on each form, calculated the mean and standard deviation of the difficulty of those items, and then identified the subset of those N items with $SD(d) < v_{min}$: these comprised the $k^{th}$ Swadesh list, as shown in Figure 1 (top).
For each candidate Swadesh list, we evaluated the total test information[^4] yielded by that Swadesh list for each of the 26 CDI:WS forms. 
...
These S-CDI items were strongly predictive of full CDI:WS scores, with an average $r=0.995$.
The mean difficulty of the S-CDI items was also quite close to the difficulty of the other uni-lemmas in the set ($M(d_{Swad})=0.31$; $M(d_{other})=0.36$), suggesting that the S-CDI items are fairly representative, and may not suffer much from ceiling effects.
...
[^4]: Total test information is the integral across a range of test-taker abilities (here, $\theta \in \{-4,4\}$) of the information yielded on a test, calculated using the IRT difficulty and discrimination parameters for those test items.
...
```{r grid-search-plots, fig.cap=c("Number of uni-lemmas appearing on at least $k$ lists (top). Total test information for Swadesh lists, and random tests of the same length (bottom)."), fig.height=5.5, fig.width=3.4}
...
The `r nrow(good_prod)` Swadesh-CDI items represent 21 of the 22 semantic categories present on the original American English CDI:WS form, with action words, animals, household, food/drink, body parts, and descriptive words being most prevalent, and connecting words, quantifiers, helping verbs, and articles being rare, and question words being unrepresented.
54% of the Swadesh CDI concepts are nouns, 22% are predicates (e.g., verbs, adjectives), 17% are function words, and 7% are other.
This breakdown is comparable to the lexical category percentages on the 680-item English CDI:WS (46% nouns, 24% predicates, 15% function words, and 5% other), suggesting that the S-CDI list is not showing a particular lexical category bias.
...
Finally, 28 S-CDI uni-lemmas are not present on the American English CDI:WS (e.g., _fruit_, _spider_, _enter_, _near_), suggesting that this method may help us move beyond an English-centric approach to studying early language.

Figure 2 shows the average cross-linguistic difficulty of CDI items by semantic category, grouped by Swadesh and non-Swadesh uni-lemmas. The difficulty of Swadesh items generally tracks with that of non-Swadesh items, although there are cases where one or the other is more/less difficult than the other.


```{r difficulty-by-category, fig.env = "figure", fig.pos = "h", fig.height=4.0, fig.align = "center", fig.cap = "Mean cross-linguistic difficulty of CDI words by semantic category, showing that selection of Swadesh concepts broadly maintained representative difficulty. Bars represent bootstrapped 95\\% confidence intervals."}
...
# mean difficulty of Swadesh words: .11; mean easiness of other words: 0.53
...
Comparing the IRT parameters of the Swadesh CDI uni-lemmas to the rest of the items showed that the discrimination parameter (i.e., slope) of the Swadesh items did not significantly differ from the others, suggesting that the Swadesh items will measure ability as well as non-Swadesh uni-lemmas.
However, the candidate Swadesh items were significantly easier than the other uni-lemmas (mean Swadesh $d=$ `r round(tmp[2,"d"], 2)`, other items' mean $d=$ `r round(tmp[1,"d"], 2)`, `r report_ttest(swad_diff_ttest)`).
...
How well do simulated raw scores from the Swadesh CDI items correlate with full CDI:WS scores?
On average, for the 26 large CDI:WS datasets, the Swadesh CDI's sumscores were strongly related to the full CDI:WS scores (mean $r=0.990$; $min=0.976$, $max=0.996$; full table on [OSF](https://osf.io/8swhb/?view_only=6f6ab9818f2a4bb288e05ca9e12f540c)).
These correlations were compared to a random baseline in which uni-lemmas were selected from the set of unique uni-lemmas, weighted by the frequency of uni-lemmas' appearance on the existing CDI:WS forms. 
The correlations of scores on these random tests vs. the full CDI:WS were quite similar to the Swadesh list (TODO SHOW THESE), but the random tests also yielded lower total test information.
Next, we evaluate the Swadesh CDI's performance on these metrics in a test of generalization to eight more CDI:WS datasets.
...
For the eight low-data languages, a comparison of simulated S-CDI scores to full CDI:WS revealed that the Swadesh CDI's raw scores were again strongly related (mean $r=0.990$; $min=0.983$, $max=0.997$).
As with the training set, however, the Swadesh list correlations were slightly but significantly lower than scores based on randomly-selected items (mean $r=0.995$; $min=0.987$, $max=0.997$; $t(7) = 3.22$, $p=.01$).
...
# TODO: re-run with the weighted random selection (this is currently the strong baseline)
...
# t = -7.0483, df = 25, p-value = 2.181e-07

t.test(gen_xx$`Swadesh r` - gen_xx$`Rand r`) 
# t = -4.2937, df = 7, p-value = 0.003594
...
28 of the Swadesh candidates are on the 100-item CDI:WS short (form A), and 26 of the Swadesh candidates are on the WS short (form B).
...
```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("NOT UPDATED The most frequently-selected 100 Swadesh CDI concepts by semantic category? (No room for 229 words, I think.."), echo=FALSE}
...
This study compared psychometric models fitted to 26 CDI datasets in order to find concepts that have low variability in their cross-linguistic difficulty, and that are frequently included on CDI:WS forms. 
We identified 229 concepts that appeared on at least 9 of the CDIs, and which have more consistent cross-linguistic difficulty than other concepts appearing on multiple CDIs.
Using real-data simulations, we showed that administering this set of "Swadesh CDI" items would generate scores that are strongly related to full CDI:WS scores, both for the original 26 datasets, and in a generalization test to eight low-data languages.
Moreover, the Swadesh CDI items result in higher total test information than a randomly-selected test of the same length. <!-- mention different types of random selection we tried: frequency-weighted, random from English... -->
...
In particular, researchers should seek to add contextually appropriate items from the categories that were less well-represented on the Swadesh list: quantifiers, .. 
