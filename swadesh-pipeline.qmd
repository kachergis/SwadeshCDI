...
fit_stats <- tibble(language = languages,
                    participants = n_subj,
                    items = n_item,
                    iterations = n_iter)
saveRDS(fit_stats, "data/fit_stats.rds")
```
...
t.test(subset(cv_res_sum, sublist=="Swadesh" & k==20)$fscore_cor - 
       subset(cv_res_sum, sublist=="Random" & k==20)$fscore_cor) # .003, n.s.

cv_res_sum %>% filter(k==20) %>%
